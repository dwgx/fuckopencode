...
> **已退役（2026-08-15）**：`fuckopencode-shield` 服务已停止，网关改监听
> `0.0.0.0:8787`（原盾端口），FurCDN 直连网关，链路中不再有盾。本文档保留
> 供回滚/参考，**勿再按它部署盾**；相关运维脚本已标注退役。
...
- 客户端自身的错误（401 之类）**非管理路径**不透传原文，统一回 503。这是
  2026-08-07 的既有决策（任何错误都不返回给客户端，号池会自然恢复），不是 bug；
  **2026-08-13 起管理/面板路径（`/__admin*`、`/__dash*`、`/__metrics`）的 401/403
  直接透传**（是业务状态：未登录/无权，盾不重试，用户要立刻看到「密码错了」；
  kiro_shield.py:737-746）；非管理路径的 4xx 仍拦截
  （CLIENT_FAULT_STATUSES = {401, 404, 405, 413, 414, 431}，kiro_shield.py:223）。
